...
TreeGrid = function(_config){
	...
	drowRowData = function(_rows, _cols, _level, _pid){
		...
		for(var i=0;i<_rows.length;i++){
			...
			s += "<tr id='TR" + id + "' pid='" + ((_pid=="")?"":("TR"+_pid)) + "' show='true' data=\"" + TreeGrid.json2str(row) + "\" rowIndex='" + rownum++ + "'>";
	...
	init = function(){
		...
		__root.find("img[folder='Y']").bind("click", function(){
			...
			var isOpen = __root.find("#" + trid).attr("show");
			isOpen = 'true'===isOpen  ? 'false' : 'true';
			__root.find("#" + trid).attr("show", isOpen);
	...
	showSubs = function(_trid){
		var isOpen = __root.find("#" + _trid).attr("show");
		if(isOpen){
...
function TreeGridItem (_root, _rowId, _rowIndex, _rowData){
	...
	this.getChildren = function(){
		...
		for(var i=0;i<trs.length;i++){
			var tr = trs[i];
			arr.push(new TreeGridItem(__root, tr.id, tr.rowIndex, TreeGrid.str2json(tr.getAttribute('data'))));
		}
		return arr;
	}
};